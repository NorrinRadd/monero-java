...
if [ -z $SKIP_MCPP ]; then
    ...
    if [ $BUILD_BOTH_ARCHS == 1 ]; then
        ...
        SKIP_MP=$SKIP_MP TARGET=$TARGET ./bin/build_libmonero_cpp.sh &&
        cd ../..
    else
        cd ./external/monero-cpp/ &&
        ARCH=$CPU SKIP_MP=$SKIP_MP TARGET=$TARGET ./bin/build_libmonero_cpp.sh &&
        cd ../..
    fi
    mkdir build
    if [ $BUILD_BOTH_ARCHS==1 ]; then
        cp external/monero-cpp/build/darwin/libmonero-cpp.dylib ./build
    else
        cp external/monero-cpp/build/${VERSION}/release/libmonero-cpp* ./build
    fi
else
    mkdir temp_cpp
...
if [ $CUR_OS == "Linux" ] && (
    ...
    toolchainBuildJNI () {
        ...
        cd build && 
        cmake -D TARGET=$1 -D STATIC=ON -D CMAKE_BUILD_TYPE=Release -D CMAKE_TOOLCHAIN_FILE=../$TOOLCHAIN_FILE .. &&
    ...
    if [ $BUILD_BOTH_ARCHS == 1 ]; then
        ...
        mv build/libmonero-java.dylib "build/x86_64-${VENDOR}-${OS}/release"
        cp build/libmonero-cpp.dylib "build/x86_64-${VENDOR}-${OS}/release"

        # aarch64
        toolchainBuildJNI "aarch64-apple-darwin11"
        mv build/libmonero-java.dylib "build/aarch64-${VENDOR}-${OS}/release"
        ...
        external/monero-cpp/external/monero-project/contrib/depends/${CURRENT_ARCH}-apple-darwin11/native/bin/${CURRENT_ARCH}-apple-darwin11-lipo -create -output build/darwin/release/libmonero-java.dylib build/darwin/aarch64/release/libmonero-java.dylib build/darwin/x86_64/release/libmonero-java.dylib
        cp build/libmonero-cpp.dylib build/darwin/release

    else
        # Build 1 archive using Depends
        toolchainBuildJNI $VERSION
        mv build/libmonero-java* build/${VERSION}/release
